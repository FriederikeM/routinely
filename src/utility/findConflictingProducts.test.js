import { findConflictingProductsIds } from "./findConflictingProducts";
...
test("gives back an array of integers that represents the id of a conflicting product when supplied with an array of products, the time of day, the name of the weekday and an array of conflicting products", () => {
  ...
  const result = findConflictingProductsIds(
    userCheckedDay,
    productsArray,
...
test("gives back an empty array if the array of conflicts doesn't include an id of the array of products from LS", () => {
  // GIVEN an array of products saved in LS, the name of the day that was clicked, the time of the day that was clicked, and an array of conflicting ids for the clicked product
  const conflicts = [3, 4];
  const userCheckedDay = "Monday";
  const userCheckedTimeOfDay = "morning";
